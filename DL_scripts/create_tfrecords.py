        ...
        with open(args.input_fastq) as handle:
            ...
            if args.num_reads:
                if len(reads) < args.num_reads:
                    reads += reads[:len(reads)-args.num_reads]
                else:
                    reads = reads[:args.num_reads]
...
def main():
    ...
    parser.add_argument('--num_reads', default=15000, type=int, help="number of reads to convert to tfrecords")
