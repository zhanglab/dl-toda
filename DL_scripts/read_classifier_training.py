...
def training_step(model_type, bert_step, data, num_labels, train_accuracy_2, train_accuracy_3, loss, train_loss_2, opt, model, first_batch):
    ...
    with tf.GradientTape() as tape:
        if model_type == 'BERT' and bert_step == "finetuning":
            ...
            loss_value, masked_lm_probs, accuracy, predictions, equal_values, embedding_table  = model(input_ids, input_mask, token_type_ids, masked_lm_positions, masked_lm_weights, masked_lm_ids, nsp_label, training)
        else:
            reads, labels = data
    ...
    return loss_value, masked_lm_probs, accuracy, predictions, equal_values, embedding_table
...
def main():
    ...
    for batch, data in enumerate(train_input.take(num_train_steps), 1):
        ...
        input_ids, input_mask, token_type_ids, masked_lm_positions, masked_lm_weights, masked_lm_ids, nsp_label = data
        loss_value, masked_lm_probs, accuracy, predictions, equal_values, embedding_table = training_step(args.model_type, args.bert_step, data, num_labels, train_accuracy_2, train_accuracy_3, loss, train_loss_2, opt, model, batch == 1)
        print('values for batch', batch, ':', loss_value, masked_lm_probs, accuracy, predictions, equal_values, embedding_table)
