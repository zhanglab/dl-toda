...
def main():
    ...
    for batch, data in enumerate(train_input.take(nstep_per_epoch*args.epochs), 1):
        ...
        if batch % 10 == 0 and hvd.rank() == 0:
            print(f'Epoch: {epoch} - Step: {batch} - learning rate: {opt.learning_rate.numpy()} - Training loss: {loss_value_1}\r{loss_value_2} - Training accuracy: {train_accuracy_1.result().numpy()*100}\t{train_accuracy_2.result().numpy()*100}\t{train_accuracy_3.result().numpy()*100}\t{train_accuracy_4.result().numpy()*100}')
        if batch % 1 == 0 and hvd.rank() == 0:
            ...
            with writer.as_default():
                ...
                tf.summary.scalar("train_accuracy_4", train_accuracy_4.result().numpy(), step=batch)
                writer.flush()
            # td_writer.write(f'{epoch}\t{batch}\t{opt.learning_rate.numpy()}\t{loss_value}\t{train_accuracy.result().numpy()}\n')
            td_writer.write(f'{epoch}\t{batch}\t{opt.learning_rate.numpy()}\t{loss_value_1}\t{loss_value_2}\t{train_accuracy_1.result().numpy()}\t{train_accuracy_2.result().numpy()}\t{train_accuracy_3.result().numpy()}\t{train_accuracy_4.result().numpy()}\n')
