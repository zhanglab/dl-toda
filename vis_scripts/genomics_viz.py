...
def RunBlast(args, query, subject=None, db=False):
	if db:
		process = subprocess.Popen(['cat', query, '|', parallel_exec, '-j', f'{args.num_processes}', blastn_exec, '-query', '{}', '-db', '/datasets/bio/ncbi-db/2025-01-26/nt', '-out', f'{args.output_dir}/mapping/test_fp_blastn.out',
